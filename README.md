...
- 旧运营系统合并：已提供七类历史活动事实的隐私安全、幂等迁移通道，并接入学员档案时间线。
...
- 现行工程执行：[工程工作流](docs/engineering-workflow.md)（明确替代 V1.1 中的旧执行条款）
- 首批任务历史：`docs/tasks/首批8个Codex任务.md`，其中状态不作为当前生产事实
- 本地统一记忆库：`E:\盛和塾运营系统\Codex记忆`，不在本仓库建立第二套记忆
...
3. 生产迁移与数据写入按工程工作流核验独立授权、备份和安全恢复；普通代码发布不包含这些动作。
