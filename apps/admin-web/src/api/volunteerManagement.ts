...
export const getVolunteerServiceUnits = (params?: Record<string, unknown>) =>
  http.request<{ success: boolean; data: VolunteerServiceUnit[] }>(
    "get",
    `${base}/service-units`,
    { params }
  );

export const createVolunteerServiceUnit = (
  data: Omit<
    VolunteerServiceUnit,
    | "id"
    | "system_name"
    | "line_name"
    | "parent_name"
...
export const getVolunteerPositionOptions = (service_unit_id: string) =>
  http.request<{
    success: boolean;
    data: {
      service_unit: VolunteerServiceUnit;
      positions: VolunteerPositionOption[];
    };
  }>("get", `${base}/position-options`, { params: { service_unit_id } });
...
export const getVolunteerMemberEditorCatalog = () =>
  http.request<{
    ...
    data: {
      positions: VolunteerPositionOption[];
      service_units: VolunteerMemberEditorServiceUnit[];
    };
...
export const createVolunteerAppointment = (data: {
  ...
  http.request<{
    success: boolean;
    data: { id: number; recommendations: VolunteerRecommendationRule[] };
  }>("post", `${base}/appointments`, { data });

export const changeVolunteerAppointmentStatus = (
  id: number,
  data: {
    status: "ACTIVE" | "SUSPENDED" | "ENDED" | "REVOKED";
    reason?: string;
  }
) =>
...
export const getVolunteerRecommendationRules = (
  params?: Record<string, unknown>
) =>
  http.request<{ success: boolean; data: VolunteerRecommendationRule[] }>(
    "get",
...
export const acceptVolunteerRecommendation = (
