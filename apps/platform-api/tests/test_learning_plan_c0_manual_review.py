...
def test_c0_keeps_only_the_remaining_template_exceptions_after_confirmation() -> None:
    review = _review()
    summary = review["summary"]

    assert summary["canonical_item_count"] == 0
    assert summary["mapping_conflict_count"] == 0
    assert summary["mapping_missing_count"] == 0
    assert summary["mapped_count"] == 144
    assert summary["safe_auto_fix_count"] == 0
    assert summary["canonical_review_required_count"] == 0
    assert summary["priority_counts"] == {"P0": 0, "P1": 0, "P2": 5}
    assert summary["template_only_supplemental_count"] == 5
    assert summary["actual_projection_status"] == "NOT_PROVIDED"
    assert len(review["items"]) == 0
    assert len(review["template_only_supplemental_items"]) == 5
...
def test_c0_markdown_contains_business_confirmation_and_b2_invariant() -> None:
    ...
    assert "0 个冲突 + 0 个缺失 = 0 个审核项" in markdown
    assert "班会顺延只能改变计划/实际时间" in markdown
    assert markdown.count("是否必须人工确认") == 0
