...
6. 候选只能用 `ReleaseType=GRAY` 创建。候选状态为 `normal` 后，先通过 `DescribeVersionDetail(candidate)` 逐字段比较 `VpcId`、`VpcCIDR`、`SubnetId`、`SubnetCIDR`；不一致即 `CANDIDATE_VPC_MISMATCH`，在任何 candidate 路由前停止。随后使用 `ReleaseGray` 的 `URL_PARAMS` 一次性 token 定向访问候选，稳定版本保持默认；依次验证 `/api/v1/system/build-info` 的批准提交、`/health/live` 和真实访问数据库的 `/api/v1/health` 20/20。普通域名的随机百分比响应不能作为候选健康证明。
...
10. API 发布依次记录 `DISCOVER_STABLE → READ_STABLE_VERSION → BUILD_PLAN → CREATE_CANDIDATE → VERIFY_CANDIDATE_CONFIG → TARGETED_HEALTH → READY_FOR_RELEASE → GRAY → FULL → VERIFIED`；按批准范围可在 `READY_FOR_RELEASE` 或 `GRAY` 停止。任何失败进入 `BLOCKED`／`FAILED`，不让失败候选获得普通流量。CLI/API 返回任务 ID 只代表已提交，仍须核验实际版本、配置、流量和业务端点。
