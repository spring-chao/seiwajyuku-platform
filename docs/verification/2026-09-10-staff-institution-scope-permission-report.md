...
本轮先通过 CloudBase 生产数据库只读查询核对机构与组织主数据：
...
常州塾、无锡塾当前没有生产机构记录，不能在本轮伪造机构或组织树。
...
- 因此江南、常州、无锡的组织根和下级树属于“主数据未落地”，不是前端筛选遗漏。
...
- 常州塾、无锡塾及江南正式组织根仍需业务主数据确认和落地；代码已能报告缺口并阻止无根机构产生有效负责范围。
- 本轮尚未执行生产部署、生产数据库迁移或生产权限写入。
- 下一步可在确认主数据方案后部署代码与 0055 迁移，再使用苏州真实工作人员和多岗位学长进行业务验收；不应把缺失的组织树用临时名称补齐。
