...
-- This migration is deliberately narrow and idempotent. It preserves the
-- class ID, all child groups, member relations, events, and historical data.
...
SELECT
    ...
    'SUCCESS',
    JSON_OBJECT('parent_id', c.parent_id),
    JSON_OBJECT('parent_id', 'org-wujiang', 'rollback_parent_id', c.parent_id),
    UTC_TIMESTAMP()
FROM org_units c
WHERE c.id = 'org-wuyue-3'
...
UPDATE org_units
SET parent_id = 'org-wujiang', updated_at = UTC_TIMESTAMP()
