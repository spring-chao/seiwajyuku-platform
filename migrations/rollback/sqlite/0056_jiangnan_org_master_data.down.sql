-- 0056 rollback is loss-averse. SQLite foreign keys intentionally refuse this
-- rollback once any new root/center is referenced by business data.
BEGIN;

CREATE TEMP TABLE jiangnan_org_master_rollback_guard (n INTEGER CHECK(n=0));
...
SELECT
  ...
  + (SELECT COUNT(*) FROM volunteer_service_units WHERE home_shuku_org_unit_id IN (
        ...
        'org-changzhou-jing-kai','org-changzhou-xin-bei','org-changzhou-jian-xing',
        'org-changzhou-long-cheng'));
DROP TABLE jiangnan_org_master_rollback_guard;
