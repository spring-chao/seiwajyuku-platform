...
-- SQLite 无法删除 0030 表内的 UNIQUE(plan_version_id, cycle_index)，因此重建
-- learning_plan_cycles 及其直接依赖表，并原样保留既有通用轨道数据。

PRAGMA foreign_keys=OFF;
BEGIN;

ALTER TABLE group_learning_cycle_tasks RENAME TO group_learning_cycle_tasks_0030;
ALTER TABLE class_learning_cycles RENAME TO class_learning_cycles_0030;
ALTER TABLE learning_plan_tasks RENAME TO learning_plan_tasks_0030;
ALTER TABLE learning_plan_cycles RENAME TO learning_plan_cycles_0030;
...
CREATE TABLE learning_plan_cycles (
    ...
    cohort_month INTEGER CHECK(cohort_month IS NULL OR cohort_month BETWEEN 1 AND 12),
...
INSERT INTO learning_plan_cycles(
    id, plan_version_id, cohort_month, cycle_index, year_index, cycle_label, created_at, updated_at
)
SELECT id, plan_version_id, NULL, cycle_index, year_index, cycle_label, created_at, updated_at
FROM learning_plan_cycles_0030;
...
SELECT id, binding_id, class_org_unit_id, learning_cycle_index, plan_cycle_id, opened_at,
       ...
       group_meeting_policy, cycle_status, closed_at, adjustment_reason,
       source_event_group_id, created_at, updated_at
FROM class_learning_cycles_0030;
...
SELECT id, class_learning_cycle_id, group_org_unit_id, plan_task_id, task_type,
       task_title, status, completed_at, adjusted_by, note, created_at, updated_at
FROM group_learning_cycle_tasks_0030;

DROP TABLE group_learning_cycle_tasks_0030;
DROP TABLE class_learning_cycles_0030;
DROP TABLE learning_plan_tasks_0030;
DROP TABLE learning_plan_cycles_0030;

CREATE UNIQUE INDEX uq_learning_plan_cycle_track
    ON learning_plan_cycles(plan_version_id, IFNULL(cohort_month, 0), cycle_index);
